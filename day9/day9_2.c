...
int get_first_diff_sum(Array *predictions) {
    Array diffs;
    ...
    if (are_all_diffs_zero == 0 && diffs.size > 1) {
        result = predictions->arr[0] - get_first_diff_sum(&diffs);
    } else {
        result = predictions->arr[0] - diffs.arr[0];
    }
...
int main() {
    ...
    while (*c != '\0') {
        ...
        int next_prediction = get_first_diff_sum(&predictions);
